...
export function useFestivals() {
  // Get all festivals
  const { data: allFestivals, isLoading: isLoadingAll } = useQuery({
    queryKey: ['/api/festivals'],
    enabled: true
  });

  // Get today's festivals
  const { data: todayFestivals, isLoading: isLoadingToday } = useQuery({
    queryKey: ['/api/festivals/today'],
    enabled: true
  });

  // Get upcoming festivals
  const { data: upcomingFestivals, isLoading: isLoadingUpcoming } = useQuery({
  ...
  const getFestivalById = (id: number): Festival | undefined => {
    if (!allFestivals) return undefined;
